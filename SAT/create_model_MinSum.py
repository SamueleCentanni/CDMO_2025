from z3 import *
from circle_matching import circle_matchings  # deve restituire: dict {w: [(i,j), ...]}
...
def create_sts_model_MinSum(n, exactly_one_encoding, at_most_k_encoding, symmetry_breaking=True):
    """
    Create a Sports Tournament Scheduling (STS) model to minimize the sum of
    home/away imbalance across all teams using Z3.

    Args:
        n (int): Number of teams (must be even).
        exactly_one_encoding (callable): Function to enforce exactly-one constraints.
        at_most_k_encoding (callable): Function to enforce at-most-k constraints.
    ...
    # === Fixed schedule: circle matching ===
    ...
    # 1. Each match (i, j) must be assigned to exactly one period
    for (i, j) in pair_to_week:
    ...
    # 2. Each period in each week must contain exactly one match
    ...
    if symmetry_breaking:
        # SB1: Force the match between team 0 and team n-1 into the first period
        ...
        for (i, j), w in pair_to_week.items():
            if i == 0:
                if w % 2 == 0:
                    solver.add(home_vars[(i, j)])
                else:
                    solver.add(Not(home_vars[(i, j)]))
            elif j == 0:
                if w % 2 == 0:
                    solver.add(Not(home_vars[(i, j)]))
                else:
    ...
    # === Objective: minimize total home/away imbalance ===
    total_objective = Sum(diff_values)

    # --- Lower bound constraint ---
    # Since n is always even, NUM_WEEKS = n-1 is odd,
    # the minimal imbalance |H - A| per team is at least 1,
    # so the sum of differences across all teams is at least NUM_TEAMS.
    lower_bound = NUM_TEAMS
    solver.add(total_objective >= lower_bound)

    return solver, match_period_vars, home_vars, home_counts, away_counts, diff_values, total_objective, pair_to_week
...
def create_sts_model_MinSum(n, exactly_one_encoding, at_most_k_encoding, symmetry_breaking=False, use_circle_matchings=True):
    """
    Compared to the standard version, this model reduces the number of variables 
    by representing each match between two teams only once (i < j), and encoding 
    home/away information separately.
    
    The model minimizes the sum of the absolute differences between home and away games for each team.

    Parameters:
        n (int): Number of teams (must be even).
        exactly_one_encoding (Callable): Function enforcing an "exactly one" constraint over a list of Boolean variables.
        at_most_k_encoding (Callable): Function enforcing an "at most k" constraint over a list of Boolean variables.
        symmetry_breaking (bool, optional): If True, adds symmetry-breaking constraints. Default is False.
        use_circle_matchings (bool, optional): If True, adds warm start. Default is True.

    Returns:
        solver_sts (z3.Solver): The Z3 solver instance with all variables and constraints.
        match_vars (dict): Mapping (i, j, w, p) → Bool variable indicating if team i plays team j in week w and period p.
        home_vars (dict): Mapping (i, j) → Bool variable indicating if team i plays at home vs team j.
        home_counts (list): List of Int variables H_i counting the number of home games per team.
        away_counts (list): List of Int variables A_i counting the number of away games per team.
        diff_values (list): List of Int variables D_i representing |H_i - A_i| per team.
        total_objective (z3.ExprRef): Objective expression: total imbalance to minimize.
    """
    ...
    # --- Warm Start ---
    # Matches for each week are fixed thanks to circle_matching(), and the model just has to 
    # decide the periods and the home/away distribution
    ...
    if symmetry_breaking and use_circle_matchings:
        # --- SB 1 ---
        # team 1 against N-1 in first week in first period
        team_a = 0
        team_b = NUM_TEAMS-1
        ...
        for w in range(NUM_WEEKS):
            for (i, j) in week_matchings[w]:
                if i == 0:
                    if w % 2 == 0:
                        solver_sts.add(home_vars[(i, j)])
                    else:
                        solver_sts.add(Not(home_vars[(i, j)]))
                elif j == 0:
                    if w % 2 == 0:
