...
from encoding_utils import exactly_one_np, at_most_k_np

# Light model with Naive Pairwise encoding + Symmetry Breaking constraints
class STS_Optimized_BaseModel:
    def __init__(self, n, exactly_one_encoding=exactly_one_np, at_most_k_encoding=at_most_k_np):
        self.n = n
        self.NUM_TEAMS = n
        self.NUM_WEEKS = n - 1
    ...
    def solve(self, timeout_seconds, random_seed=None, verbose=False, solver_options=True):
        ...
        while True:
            ...
            if optimal_objective_value is not None:
                ...
                if verbose:
                    print(f"  Searching for solution with total_diff < {optimal_objective_value} (Remaining time: {remaining_time}s)...")
            else:
                if verbose:
            ...
            if verbose:
                print(f"  Solver result: {status} (Elapsed: {current_elapsed_time:.2f}s)")
                ...
                if optimal_objective_value is None or current_objective < optimal_objective_value:
                    ...
                    for (i, j, p), var in self.match_period_vars.items():
                        if model.evaluate(var):
                            ...
                            week = self.pair_to_week[(i, j)] + 1  # +1 for human-readable week
                            best_solution_schedule.append((home_team + 1, away_team + 1, week, p + 1))

                    if verbose:
                        print("    Home/Away counts per team:")
                    ...
                    if optimal_objective_value == 1:
                        ...
                        if verbose:
                            print("    Lower bound reached. Optimum found.")
                        self.solver.pop()
                        break
        ...
        if proven_optimal_final:
            ...
            if verbose:
                print("  No solution found (timeout or error).")
...
class STS_Optimized_BaseModel:
    ...
    def solve(self, timeout_seconds, verbose, random_seed=None, solver_options=True):
