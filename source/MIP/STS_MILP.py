...
def saveSol(result, solution, time, opt=True, solver='gurobi', filename='data.json'):
    formatted_sol = []
    for p in range(n//2):
        row = []
        for w in range(n-1):
...
if __name__ == "__main__":
    ...
    for n in range(6,18,2):
        ...
        saveSol(result, solution, end, opt=True, solver=solver, filename=f'circleMatching_{n}.json')
